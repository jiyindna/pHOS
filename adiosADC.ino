  ...
  if (Serial.available() >0) {
    ...
    switch (s) {
	  ...
	  /* the first received value indicates the mode       
           ...
           s=400 is singleEnd read from ADC
           s=110 is setGain 1x for ADC
           s=120 is setGain 2x for ADC
                                                             */
        s=10*(val-48);
      }
      ...
      if ((s>120 && s<210) || (s>250 && s!=340 && s!=400)) {
        s=-1;
      }
	...
	Serial.println(agv);       /* send value via serial  */
      ...
      /* s=110 120 means set Gain for ADC*************************** */
      
      case 110:
      if (val==59) { 
         ads1115.setGain(GAIN_ONE);       /* Set gain to 1x +/-4.096V   */  
         Serial.println(111);   
       }
      s=-1;  /* we are done with AI so next state is -1      */
      break; /* s=110 taken care of                           */
      
      case 120:
     if (val==60) { 
         ads1115.setGain(GAIN_TWO);       /* Set gain to 2x +/- 2.048V   */ 
         Serial.println(111);     
       }
      s=-1;  /* we are done with AI so next state is -1      */
      break; /* s=120 taken care of                           */
     ...
     // ads1115.setGain(GAIN_TWO);
      /* This is an auxiliary function that returns the ASCII 
        ...
        pin=val-97;                /* calculate pin          */
        int16_t agv;
        agv = ads1115.readADC_SingleEnded(pin);       /* perform ADC Input   */
	Serial.println(agv);       /* send value via serial  */
      }
      s=-1;  /* we are done with AI so next state is -1      */
